...
const STATUS_FILTERS: (BulkJobStatus | "all")[] = [
  "all", "queued", "running", "completed", "completed-with-errors", "failed", "cancelled",
];

export default function BulkInvitationJobs() {
  ...
  // Auto-refresh every 10s while running jobs exist
  useEffect(() => {
      ...
      <div className="min-h-screen bg-bluegrey-25">
        ...
        <div className="sticky top-16 bg-white border-b border-bluegrey-100 z-20 px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-xl font-semibold text-bluegrey-900">Bulk Invitation Jobs</h1>
              ...
              <table className="w-full border-collapse text-sm">
                <thead>
                  <tr className="bg-bluegrey-50 border-b border-bluegrey-200">
                    {["Job ID", "Status", "Organization", "Created by", "Created date", "Total", "Success", "Failed", "Progress", "Completed"].map((h) => (
                ...
                <tbody>
                  {filtered.map((job) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-bluegrey-800 whitespace-nowrap">{job.organization}</td>
                      <td className="px-4 py-3 text-bluegrey-600 whitespace-nowrap text-xs">{job.createdBy}</td>
                      ...
                      <td className="px-4 py-3 text-right">
                        <span className={job.failedUsers > 0 ? "text-red-600 font-medium" : "text-bluegrey-400"}>
                          {job.failedUsers}
                        </span>
                      </td>
